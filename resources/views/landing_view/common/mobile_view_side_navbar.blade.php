...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function fetchCartData() {
            ...
            fetch('/fetch-cart').then(response => response.json()).then(data => {
        ...
        function removeCartItem(cartItem) {
            ...
            // Remove item from local storage and backend
    ...
    function updateUI(sku) {
        const cartItem = document.querySelector(`.cart-item[data-id="${sku}"]`);
        if (!cartItem) return;

        const quantity = getProductsFromLocalStorage().find(p => p.sku === sku)?.quantity || 0;
        cartItem.querySelector('.action-input').value = quantity;
        cartItem.querySelector('.item-total-price').textContent = `$${(quantity * parseFloat(cartItem.querySelector('.cart-info p').textContent.replace('Unit Price - $', ''))).toFixed(2)}`;
...
<script>
    ...
    const pricePerItem = parseFloat(document.getElementById('product-price').textContent.replace(/[^\d.]/g, '')); // Ensure this is a number
    const totalPriceElement = document.getElementById('total-price');
    const actionMinus = document.querySelector('.a-minus');
    const actionPlus = document.querySelector('.a-plus');
    //Get elements end
    document.addEventListener('DOMContentLoaded', function() {
        const quantityInput = document.getElementById('quantity-input');
    ...
    // Initial calculation to set the correct total price on load
    updateTotalPrice();
